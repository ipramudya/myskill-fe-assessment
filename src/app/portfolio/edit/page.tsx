					...
					<Section className="my-2 px-3 pb-4 pt-3" isForeground>
						...
						<div className="flex flex-col space-y-4">
							<div className="flex flex-col space-y-2">
								<Label htmlFor="name" isError={Boolean(form.formState.errors.name)}>
									Nama
								</Label>
								<Input
									id="name"
							...
							<div className="flex flex-col space-y-2">
								<Label
									htmlFor="possition"
									isError={Boolean(form.formState.errors.possition)}
								>
									Title
								</Label>
								<Input
								...
								{Boolean(form.formState.errors.possition) && (
									<ErrorMessage>
										{form.formState.errors.possition?.message}
									</ErrorMessage>
								)}
							</div>
							<div className="flex flex-col space-y-2">
								<Label
									htmlFor="description"
									isError={Boolean(form.formState.errors.description)}
								>
									Deskripsi
								</Label>
								<Textarea
					...
					{form.watch("portfolio").length !== 0 ? (
						...
						<>
							{form.watch("portfolio").map((p, idx) =>
								minimizedIndex !== null && minimizedIndex.includes(p.id) ? (
									<PortfolioItem
										key={p.id}
										className="my-2"
										name={p.name}
									...
									>
										<Button
											...
											isIconButton
											className="bg-white"
											onClick={() => handleMinimizePortfolioItem(p.id)}
										>
											<Icon.Maximize className="text-black-low md:h-5 md:w-5" />
										</Button>
									</PortfolioItem>
								) : (
									<AddPortfolioForm key={p.id} id={p.id} index={idx} />
								),
							)}
							<Button
						...
						<Section className="mb-4 mt-2 p-3" isForeground>
							...
							<div className="flex flex-col space-y-2">
								<p className="text-sm text-gray">
									portofolio anda masih kosong, mohon penuhi terlebih dahulu.
								</p>
								<Button
